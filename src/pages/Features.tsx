...
const FEATURES: Feature[] = [
  ...
  { icon: ShieldCheck, title: "Adversarial Robustness Lab", category: "Research", where: "Research · Adversarial", what: "Attacks your text with meaning-preserving perturbations and reports how far predictions move.", how: "Seven CheckList-style transformations (synonym substitution, double negation, entity masking, hedge injection, character noise, casing, punctuation stripping) scored for label-set Jaccard overlap, confidence shift, attack flip rate and a composite robustness score, with CSV export." },
  { icon: Sigma, title: "Temporal Drift Dashboard", category: "Research", where: "Research · Drift", what: "Tracks whether your bias profile is shifting over time and flags the moment it changed.", how: "Daily/weekly/monthly bucketing of the local history, Population Stability Index between the first and second halves, Mann-Kendall trend tests on bias volume and confidence, and two-sided CUSUM change-point alarms, plus rising/receding bias rankings." },
  { icon: Sparkles, title: "Counterfactual Bias Rewriter", category: "Research", where: "Research · Counterfactual", what: "Produces the smallest possible edit that removes biased framing while keeping the claim intact.", how: "Fourteen audited rewrite rules mapped to bias types, with a per-edit rationale, token-level Levenshtein edit distance and a minimality score so counterfactual pairs are publishable as a dataset." },
  { icon: Users, title: "Fairness Audit", category: "Research", where: "Research · Fairness", what: "Checks whether the classifier treats group-coded text slices equally.", how: "Per-group confusion matrices with demographic parity difference, disparate impact ratio against the four-fifths rule, equal opportunity (TPR gap) and equalized odds (max of TPR/FPR gaps), over a seeded reproducible audit sample." },
  { icon: Target, title: "Active-Learning Sampler", category: "Research", where: "Research · Active Learning", what: "Recommends which unlabelled samples are worth annotating next.", how: "Predictive entropy, margin and least-confidence acquisition combined with hashed-embedding diversity via greedy max-marginal-relevance, with a tunable λ trade-off and an estimated labelling-effort saving." },
